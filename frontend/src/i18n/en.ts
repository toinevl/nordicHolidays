import type { LocaleStrings } from './types'

export const en: LocaleStrings = {
  generator: {
    panelTitle: 'Plan Your Trip',
    startCity: 'Start city',
    finishCity: 'Finish city',
    searchCity: 'Search city...',
    customCity: 'Add custom city',
    tripLength: 'Trip length (nights)',
    startDate: 'Start date (optional)',
    mustVisit: 'Must-visit places',
    pressEnter: 'Press Enter to add',
    addPlace: 'Add place',
    avoid: 'Places to avoid',
    generateBtn: 'Plan my trip',
    regenerateBtn: 'Try a new route',
    preferencesSaved: 'Saved for next time',
    generating: 'Generating...',
    country: 'Country',
    routeModeLabel: 'Route',
    routeModePoint: 'From A to B',
    routeModeDiscover: 'Surprise me',
    discoverHint: 'We pick a logical start and finish city with good car-rental access (ferry port or airport).',
    themesLabel: 'What are you looking for in this trip?',
    paceLabel: 'Pace',
    paceRelaxed: 'Relaxed — settle in',
    paceBalanced: 'Balanced',
    pacePacked: 'See a lot — move often',
  },
  themes: {
    nature: 'Nature & hiking',
    coast: 'Coast & islands',
    city: 'Cities & culture',
    food: 'Food & local flavour',
    wildlife: 'Wildlife',
    history: 'History & Vikings',
    aurora: 'Northern lights',
    family: 'Family with kids',
  },
  saved: {
    title: 'Community Trips',
    close: 'Close',
    namePlaceholder: 'Trip name...',
    save: 'Save',
    saving: 'Saving...',
    load: 'Load',
    loadingTrip: 'Loading...',
    empty: 'No trips yet — be the first to add one!',
    loading: 'Loading...',
    errorLoading: "Couldn't load trips — try again",
    saveFailed: 'Save failed',
    loadFailed: 'Load failed',
  },
  status: {
    myTrips: 'Saved Trips',
    myTripsTitle: 'View saved trips',
    generate: 'Plan my trip!',
    generateTitle: 'Plan your trip',
    share: 'Share',
    shareTitle: 'Share this trip',
    saved: 'Saved',
    unsaved: 'Unsaved',
    defaultTripName: 'My Nordic Trip',
  },
  toast: {
    generated: 'Itinerary generated',
    generationFailed: 'Generation failed: {msg}',
    loaded: 'Loaded "{name}"',
    saved: 'Saved "{name}"',
    shareCopied: 'Link copied to clipboard',
    shareFailed: 'Share failed',
    sharedItineraryLoaded: 'Shared itinerary loaded',
    sharedItineraryFailed: 'Failed to load shared itinerary',
    saveNoteFirst: 'Save your trip first, then your notes will be saved too.',
    saveNoteFailed: 'Failed to save note',
    undone: 'Last edit undone',
    undoFailed: 'Undo failed',
  },
  itinerary: {
    plannedNights: 'Planned nights',
    roadKilometres: 'Road kilometres',
    overnightStops: 'Overnight stops',
    longestDriveTo: 'Longest drive to {dest}',
    allStops: 'All stops',
    selectedStop: 'Selected stop',
    dayPrefix: 'Day',
    dayTrip: 'Day trip',
    dayTripFrom: 'Day trip from {base}',
    oneNight: '1 night',
    nights: '{n} nights',
    flyHere: 'Fly here',
    noStopsMatch: 'No stops match this filter',
    print: 'Print',
    notes: 'Notes',
    notesPlaceholder: 'Add your own notes',
    saveNote: 'Save',
    savingNote: 'Saving...',
    exportGPX: 'Export GPX',
    exportICS: 'Export iCal',
    exportGoogleMaps: 'Google Maps',
    exportWaze: 'Waze',
    tripIndex: 'Quick jump',
    undoLastEdit: 'Undo last edit',
    findHotels: 'Find hotels in {city}',
    findActivities: 'Things to do in {city}',
    rentCar: 'Rent a car for this trip',
    tripStarting: '{totalDays}-day trip from {date}',
    addStop: 'Add stop',
    confirmRemove: 'Remove {city}?',
    confirmRemoveYes: 'Yes, remove',
    confirmRemoveKeep: 'Keep',
    regenerateRoute: 'Plan a new route',
  },
  season: {
    skane: 'Mild climate. Warm summers, colourful autumns.',
    blekinge: 'Coastal archipelago. Best Jun–Aug.',
    gotland: "Sweden's sunniest island. Hot dry summers.",
    halland: 'West coast beaches. Busy Jul–Aug.',
    bohuslan: 'Rocky coast & seafood. Best Jun–Sep.',
    gothenburg: 'Maritime climate. Rain year-round, warm summers.',
    vastraGotaland: 'Maritime climate. Rain year-round, warm summers.',
    stockholm: 'Continental. Warm summers, cold winters.',
    uppland: 'Flat farmland. Hot summers, snowy winters.',
    ostergotland: 'Mild summers, good cycling terrain.',
    smaland: 'Dense forests. Cool nights even in summer.',
    varmland: 'Lakes & berries. Best Jul–Aug for hiking.',
    dalarna: 'Mountain climate. Snowy winters, cool summers.',
    jamtland: 'High altitude. Snow possible Jun & Sep.',
    harjedalen: 'Remote fells. Snow lingers into June.',
    lapland: 'Midnight sun Jun–Jul. Aurora Sep–Mar.',
    norrbotten: 'Arctic. Midnight sun and polar night.',
    vasternorrland: 'Coastal forests. Cool and quiet.',
  },
  auth: {
    signIn: 'Sign in',
    signOut: 'Sign out',
    profileSaved: 'Profile saved',
  },
  country: {
    se: 'Sweden',
    no: 'Norway',
    dk: 'Denmark',
    fi: 'Finland',
  },
  nav: {
    itinerary: 'Itinerary',
    food: 'Food',
    stay: 'Stay',
    map3d: '3D Map',
    business: 'Business',
  },
  hero: {
    flyRoute: '▶ Fly the Route',
    viewItinerary: 'View Itinerary ↓',
    scrollCue: 'Scroll to explore',
    badge: 'Road Trip · Aug 25 – Sep 14, 2026',
    subtitle: 'From the Øresund Bridge to the High Coast',
    metaDays: 'days',
    metaKm: 'km',
    metaDestinations: 'destinations',
    metaFoodRegions: 'food regions',
  },
  hero2: {
    brand: 'Fjordvia.',
    tagline: 'Your Nordic road trip, planned in minutes.',
    ctaPlan: 'Plan your trip',
    ctaSee: 'See a real trip',
    statbar: '3 countries · 6 languages · Real driving times',
    why1Title: 'No 40-tab research',
    why1Body: 'Skip the spreadsheet evenings. Tell us where you want to go — we do the route work.',
    why2Title: 'Real distances & times',
    why2Body: 'Every day plan uses actual driving times between stops, not guesswork.',
    why3Title: 'Take it with you',
    why3Body: 'Save your trip and share one link with everyone who is coming along.',
    showpieceTitle: 'This could be yours in two minutes.',
    showpieceDemo: 'A real trip, generated by Fjordvia — every stop, drive and night planned.',
    showpieceCta: 'Open the demo trip',
    howTitle: 'How it works',
    howStep1: 'Tell us your route',
    howStep2: 'Get a day-by-day plan',
    howStep3: 'Save, share, go',
    storyTitle: 'Why Fjordvia exists',
    storyBody:
      'Fjordvia started with a familiar frustration: planning a Nordic road trip meant weeks of open tabs and a spreadsheet that ate every evening. We wanted the trip — the fjords, the fika stops, the long light — without the planning grind. So we built the tool we wished existed.',
    finalTitle: 'Ready when you are.',
    finalCta: 'Plan your trip',
  },
  sections: {
    overviewLabel: 'Trip Overview',
    overviewTitle: 'At a Glance',
    overviewDesc: 'Every day, every drive, every highlight — scan the full trip or click a row for details.',
    itineraryLabel: 'Day by Day',
    itineraryTitle: 'The Full Route',
    itineraryDesc: '21 days from Malmö to the High Coast and back — medieval towns, forest lakes, Viking heritage, and off-the-beaten-track surprises.',
    itineraryDescDynamic: '{totalDays}-day trip from {startCity} to {endCity} — a curated Nordic road-trip with stops in {stopCount} cities.',
    culinaryLabel: 'Eat & Drink',
    culinaryTitle: 'Culinary Highlights',
    culinaryDesc: 'Six distinct food cultures — Gothenburg shellfish, Dalarna game, High Coast smoked fish, and Stockholm\'s new Nordic kitchen.',
    accomLabel: 'Where to Sleep',
    accomTitle: 'Accommodation Guide',
    accomDesc: 'All stops filtered for: free cancellation · private bathroom · terrace or balcony where available.',
    accomTip: '<strong>Booking tip:</strong> Use Booking.com\'s "Free cancellation" filter. For lake lodges and Höga Kusten, book 2–3 months ahead — they sell out. Search <em>gäststugor</em> (guest cottages) directly for the most authentic Swedish stays.',
    filterTitle: 'Route Focus',
  },
  accom: {
    colDestination: 'Destination',
    colType: 'Accommodation Type',
    colCancellation: 'Cancellation',
    colBathroom: 'Private Bathroom',
    colTerrace: 'Terrace / Balcony',
    colNotes: 'Notes',
  },
  map3d: {
    hint: '3D map — drag to rotate · scroll to zoom',
  },
  footer: {
    days: 'Days',
    kilometres: 'Kilometres',
    destinations: 'Destinations',
    foodRegions: 'Food Regions',
    tagline: 'Netherlands → anywhere in the Nordics',
    buildLocal: 'Build local',
    colofon: 'Colophon',
    privacy: 'Privacy',
    cookies: 'Cookies',
  },
  consent: {
    bannerText: 'We only use analytics cookies if you say yes. Everything works fine without them.',
    accept: 'OK',
    decline: 'No thanks',
    readMore: 'More info',
  },
  notes: {
    label: 'Notes',
    add: 'Add a note',
    anonymous: 'Anonymous',
    alreadyExists: 'You already have a note here — delete it first to post a new one',
    deleteConfirm: 'Delete your note?',
    deleted: 'Note deleted',
    deleteFailed: 'Could not delete the note',
    nameLabel: 'Your name (optional)',
    placeholder: 'Share a tip or note for this stop…',
    empty: 'No notes yet — be the first!',
    save: 'Post',
    saving: 'Posting…',
    posted: 'Note posted',
    postFailed: 'Could not post the note',
    loadFailed: 'Could not load notes',
    cancel: 'Cancel',
  },
  loading: {
    generating: 'Generating your itinerary...',
  },
  widget: {
    poweredBy: 'Powered by',
    planTrip: 'Plan your trip',
  },
  overview: {
    title: 'Trip Overview',
    subtitle: 'All {n} days at a glance',
    columnDay: 'Day',
    columnDate: 'Date',
    columnRoute: 'Route',
    columnDistance: 'Distance',
    columnDriveTime: 'Drive',
    columnHighlights: 'Highlights',
    stayDay: 'Stay',
    dayTrip: 'Day trip',
    dayTripShort: 'Trip',
    totalDistance: 'Total driving distance',
    noData: '\u2014',
    clickHint: 'Click a row for details',
  },
  map: {
    legendOvernight: 'Overnight stay',
    legendDayTrip: 'Day trip',
    legendRoute: 'Route',
    legendExcursion: 'Day excursion',
    loadFailedTitle: 'Map could not load',
    loadFailedBody: 'Your browser (or device) may not support WebGL. Route markers still work — scroll down for the full itinerary.',
    previewCta: 'View the full route in 3D',
    timelineTitle: 'Route stops',
    closeTimeline: 'Close stop list',
  },
  b2b: {
    kicker: 'For Business',
    title: 'Your customers plan their Nordic trip on your site',
    subtitle: 'Embed Fjordvia\u2019s AI route planner as a white-label widget. Your brand, your colours, your affiliate links — leads land in your inbox.',
    feature1Title: 'AI itinerary generator',
    feature1Body: 'Customers describe their dream Nordic road trip and get a day-by-day route with stops, maps, and driving directions — without leaving your website.',
    feature2Title: 'Your branding',
    feature2Body: 'White-label theming with your colours, logo, and domain. Your affiliate IDs on every lodging and activity link. \u201cPowered by Fjordvia\u201d in the footer.',
    feature3Title: 'Lead capture',
    feature3Body: 'When a visitor saves their itinerary, their email goes to your CRM. GDPR consent checkbox built in. No more lost leads.',
    demoLabel: 'Live demo',
    pricingLabel: 'Pricing',
    pilotBadge: 'Pilot',
    perMonth: '/mo',
    pilotDuration: '3-month pilot, cancel anytime',
    pilotFeature1: 'Embeddable widget on your site',
    pilotFeature2: 'Up to 500 AI generates / month',
    pilotFeature3: 'Your affiliate links + lead capture',
    pilotFeature4: 'Email support',
    pilotSubject: 'Fjordvia pilot enquiry',
    startPilot: 'Start pilot',
    standardBadge: 'Standard',
    standardDuration: 'Monthly or annual (10\u00d7 monthly)',
    standardFeature1: 'Everything in Pilot',
    standardFeature2: 'Unlimited generates',
    standardFeature3: 'Custom theming + custom domain',
    standardFeature4: 'Priority support (24h response)',
    standardFeature5: 'Quarterly analytics report',
    standardSubject: 'Fjordvia demo request',
    bookDemo: 'Book a demo',
    caseStudiesLabel: 'Case studies',
    caseStudiesEmpty: 'Be among the first partners. Pilot slots available now.',
    demoIframeTitle: 'Fjordvia live demo',
  },
  validation: {
    selectStartCity: 'Please select a start city',
    selectFinishCity: 'Please select a finish city',
    needBothCities: 'Fill in both a start and finish city, or choose "Surprise me".',
    minDuration: 'Minimum trip duration is 7 days',
    generationFailed: 'Trip generation failed — please try a different route or fewer stops.',
    rateLimit: 'Too many requests. Wait a moment and try again.',
    authFailed: 'Sign-in required to save trips.',
  },
  culinary: {
    mustTry: 'Must try',
  },
  accomPolicy: {
    free: 'Free cancellation',
    cond: 'Conditional',
    mod: 'Moderate',
  },
  tags: {
    offbeat: 'Off-beat',
  },
  aria: {
    routeFilters: 'Filter your trip',
    removeTag: 'Remove {city}',
    stopMarker: '{city}, stop {n}',
    dayTripMarker: 'Day trip near {base}',
    mapLegend: 'Map legend',
    mapLabel: 'Nordic road trip map',
    map3dLabel: '3D map of Nordic road trip',
    localeDropdown: 'Choose your language',
  },
  gallery: {
    title: 'Community Trips',
    loadTrip: 'Load trip: {title}',
  },
  creator: {
    title: '{name} — Creator Profile',
    anonymous: 'Anonymous Creator',
    created: 'Created',
    updated: 'Updated',
    notFound: 'Profile not found.',
    loadError: 'Failed to load profile.',
  },
}
