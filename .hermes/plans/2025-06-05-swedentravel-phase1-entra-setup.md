# Phase 1 Entra Setup Execution Plan

> **For Hermes:** This is the executable slice of `2025-06-05-swedentravel-entra-personalization.md` Phase 1.

**Goal:** Create a single source of truth for Entra app registration that can be executed once and reused across environments.

**Deliverable:** `docs/entra-setup.md`

---

### Task 1: Create docs/entra-setup.md

**Objective:** Capture tenant id, client id, and other Entra registration artifacts needed for frontend and API auth.
...
**Step 2: Create placeholder sample**

```bash
git add docs/entra-setup.md
```

Only this single step belongs in Phase 1.
...
- `docs/entra-setup.md` exists under the docs tree.
- The file contains tenant, redirect URI, scope, client id, tenant id, and validation steps.

## Risks And Questions

- The callback path `/.auth/login/aad/callback` assumes SWA integration. If public paths differ, update the runbook before tenant registration.
- If the tenant already exists, reusing an existing app registration may simplify onboarding.
```
