...
**Authorization & isolation.** The API resolves the owner server-side (`resolveOwnerId`) and uses it as the Table partition key for every read/write. Cross-guest access is impossible by construction — no endpoint accepts an arbitrary partition key.
