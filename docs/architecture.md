...
- **Itineraries (public reads, edit-token writes — #47, #146, #147):** `resolveOwnerId` is still not called — there is no account or owner binding. **Reads and shares stay fully public:** `list` and `get` need nothing, every itinerary lives under one shared partition key, and any visitor can view any itinerary via its id/share URL. **Writes are gated by a per-itinerary edit-token (#146):** `POST /api/itineraries` mints `editToken = base64url(randomBytes(32))`, persists **only** `editTokenHash = sha256(token)` (hex) on the entity, and returns the raw token in the create response body exactly once (write-once — never stored in plaintext, unrecoverable if lost). The frontend stashes it in `localStorage["fjordvia:edit:<id>"]` and replays it as the `X-Edit-Token` header on `PATCH` and `POST .../undo`; the handlers re-hash and compare in constant time (`crypto.timingSafeEqual`, `api/src/lib/editToken.ts`). A mismatched/absent token → `403 {code:"edit_token_invalid"}`. This is *not* identity: whoever holds the token (e.g. via a shared link that includes it) can edit; it exists to stop *anonymous drive-by vandalism* of another visitor's saved trip, not to authenticate a person.
  - **Legacy tokenless itineraries (#147):** entities saved before #146 carry no `editTokenHash`. Current behaviour is a **hard `403 {code:"legacy_no_token"}`** on any write — they are read-only. **Open product question:** hard-403 vs. a one-time "claim" flow (first caller to present a token binds it) vs. leaving legacy entities freely editable. Not yet decided; the hard-403 is the conservative default, and the frontend's follow-up is a "duplicate as a new trip" affordance (owned by a separate lane).
  - **Write rate limiting (#148):** `create` / `patch` / `undo` all call `checkAndIncrementItineraryWriteRateLimit` (`api/src/lib/rateLimit.ts`) — per-`X-Owner-Id` (10/h, spoofable, best-effort) **and** per-IP (30/h, last `X-Forwarded-For` hop) buckets → `429` + ASCII-only `Retry-After`. There is no `DELETE` endpoint.
  - **Undo (#51):** every `PATCH` snapshots the pre-patch entity into `previousStateJson`; `POST /api/itineraries/{id}/undo` restores it once (single-level, not full history).
  - **Content moderation:** `POST`/`PATCH` bodies are Zod-validated for shape, not content — trip names/city text are still an unmoderated public surface (persisted, later rendered to other visitors; XSS mitigated by output escaping, see Hardening below), but the edit-token + rate limits now put real friction in an abuser's way.
...
- Rate limiting: per-owner + per-IP counters in the `RateLimits` table (`checkAndIncrementRateLimit` for `/api/generate`, `checkAndIncrementItineraryWriteRateLimit` for itinerary create/patch/undo, plus track/partner/lead limiters) → 429 with ASCII-only `Retry-After`.
- Itinerary write protection: per-itinerary edit-token (`X-Edit-Token`), sha256-hashed at rest, constant-time verified (#146); legacy tokenless entities are read-only (#147).
- Guest id 30-day rolling expiry; profile schema hardening strips internal fields on PUT.

**Open items:** (a) legacy tokenless itineraries are hard-`403` read-only on writes (#147) — the claim-flow-vs-hard-403 product decision is still open, and the frontend "duplicate as new" fallback UX is a separate lane; (b) itinerary bodies are shape-validated but not content-moderated (#47); (c) the dead `nordicholidays.azurestaticapps.net` origin is still allow-listed and could be removed.
