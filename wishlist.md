...
- [~] (A) Day-trip derivation helper: `isDayTrip`/`baseFor` pure functions in new `frontend/src/lib/dayTrips.ts`, nearest-preceding-else-following base resolution, null-safe for all-day-trip data +feature +ui @me #59
- [ ] (A) Map distinction: solid main route threads only overnight bases; new dashed `route-excursions` MultiLineString layer for base→day-trip spokes (no dash animation!); hollow-diamond `.map-marker--daytrip` markers; i18n'd legend overlay — `frontend/src/components/MapView.ts` + `mapGeometry.ts` (new, pure, testable) + `main.css` +feature +ui @me #60
- [ ] (B) Itinerary list: `◇ Day trip` badge + "Day trip from {base}" line + nested/lighter card styling for `nights === 0` stops; new i18n keys (dayTripFrom, map.legend*) in EN/NL/DE with completeness test — `frontend/src/components/ItineraryView.ts`, `frontend/src/i18n/*` +feature +ui +i18n @me #61
- [ ] (B) Generator: LLM tool schema currently says stops are all overnight (`api/src/lib/itinerarySchema.ts:17`) — describe nights=0 day trips, add hub-and-spoke SYSTEM_PROMPT guidance (2-3-night bases, ≤1.5h out-and-back, first/last stop overnight), normalize a nights-0 first stop to 1 in `generate.ts` +feature +api @me #62
