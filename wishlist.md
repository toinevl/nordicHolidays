...
- [~] (B) `listItinerariesHandler` scans the Itineraries table without a `select` projection, so Table Storage ships every row's full `itineraryJson`, 48KB `thumbnail`, and `previousStateJson` to the function even though the response strips them — harmless at 5 rows, degrades as the shared no-delete table grows; add a column projection to the query +perf +api @me #56
